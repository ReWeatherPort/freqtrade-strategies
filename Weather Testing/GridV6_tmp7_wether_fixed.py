...
class GridV6_tmp7_wether_fixed(IStrategy):
    INTERFACE_VERSION = 2
    ...
    # 注意：以下四個參數用新版名稱
    use_exit_signal = True                 # 原 use_sell_signal
    exit_profit_only = False               # 原 sell_profit_only
    exit_profit_offset = 0.01              # 原 sell_profit_offset
    ignore_roi_if_entry_signal = False     # 原 ignore_roi_if_buy_signal
    ...
    # ★★★ 新增：最小化加上 buy_params (示例) ★★★
    buy_params = {
        # 可以真正放你想要調整的 Buy 超參數關鍵值
        # 這裡先示範一個簡單參數，避免 "No params for buy found" 提示
        "dummy_buy_param": 1
    }

    # ★★★ 新增：最小化加上 protection_params (示例) ★★★
    protection_params = {
        # 如果策略沒特別定義保護機制，可只放空或示範用參數
        "dummy_protection": 0
    }

    # 這裡是原本的 DecimalParameter 定義
    grid_down_spacing_pct = DecimalParameter(
        3.0, 5.0, default=0.4, decimals=1, load=True, space='sell', optimize=False
    )
    ...
    def calculate_state(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # --------------------------------------------------------------------------------
        # BUG FIX: 以 row 的數量 (len(dataframe) - 1) 作為最後一個可迭代下標
        # 而不是直接 dataframe.tail(1).index.item()，以免遇到非 0-based 或非整數索引出錯。
        # --------------------------------------------------------------------------------
        ...
        # merge
        dataframe = pd.concat([
            dataframe, df_buy_1, df_buy_2, df_sell_1,
            df_grid_up, df_grid_down, df_bot_state
        ], axis=1)
